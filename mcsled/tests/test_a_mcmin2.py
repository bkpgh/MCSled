...
Adds a local minimum that must be escaped.
"""
...
def test_integrated():
    ...
    # Create the system
    mysys = System(minxy,maxxy,xinit,yinit,ranseed=9797973)
    print("Starting System: ")
    (x,y) = mysys.get_state()
    print("x,y = ",x,y)
    print("Energy = ",mysys.get_energy())
    print()

    # Set up the Annealing schedule and the Simulation
    schedule = mcsled.AnnealingSchedule(Ti=10.0,Tf=0.0005,reduce=0.96,
                                        ncycles=200,nstop=25)
    sim = mcsled.MCSim(mysys,schedule)
